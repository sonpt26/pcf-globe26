...
Experiments on a 14-UE smart factory scenario with 5 controlled QoS stress tests demonstrate that PCF-MORL achieves 51\% lower violation rate and 31\% faster recovery than the best threshold baseline, while providing a rich Pareto front of 15 QoS operating points that static and single-objective methods cannot reach.
\end{abstract}
...
When operator priorities shift (\eg, day-shift to night-shift), the system adapts in ${<}5$\,ms, achieving ${\geq}$99\% of dedicated-training quality at zero additional cost.
...
\end{align}
where $\text{rate}_\text{urllc} \in \{5, 10, 15, 20\}$\,Mbps and $\text{rate}_\text{embb}$ takes 19 values from 2 to 100\,Mbps, with fine 1\,Mbps steps in the 2--10\,Mbps range (near the per-UE capacity ceiling of ${\sim}$10\,Mbps on the 25\,MHz BWP) and coarser steps above.
Rate ranges straddle typical application demand so that each level produces meaningfully different QoS outcomes: URLLC rate spans from throttled-burst (5\,Mbps) to unrestricted (20\,Mbps); eMBB rate spans from heavy cap (2\,Mbps) to full quality (100\,Mbps).
...
\end{align}
where $T_\text{target} = 12$\,Mbps (per-UE achievable throughput on the 25\,MHz BWP), and $E_\text{ref}$ is the energy-per-bit under a fixed midpoint policy (measured once before training).
...
We implement using the MORL-Baselines toolkit~\cite{morlbaselines}.
...
\emph{(3)~ns-3 instances} ($P$ processes): each worker owns one independent simulation instance communicating via stdin/stdout JSON pipe with a unique random seed.
...
$\gamma{=}0.99$, learning rate $3{\times}10^{-4}$, network $[256]^4$, batch~128, replay buffer $5 \times 10^5$, SF sync interval $K{=}200$ episodes, $\varepsilon{=}0.01$.
...
\input{tabs/tab3_qos_e2}

Table~\ref{tab:qos_e2} shows per-scenario QoS comparison on E2 (URLLC burst).
PCF-MORL achieves the lowest VR (0.051), TTR (1.24~steps), and MVD (2.4~steps)---a 51\% VR reduction and 31\% TTR improvement over the best threshold baseline (A3 Hysteresis).
This advantage stems from proactive action: PCF-MORL detects load trends across all 12~state features before violations occur, whereas threshold baselines react only after delay budgets are breached.
A2~Aggressive achieves the highest throughput (9.80\,Mbps) but at the cost of severe delay violations (VR$=$0.286, MVD$=$12).
A1~Conservative protects delay (VR$=$0.105) but sacrifices throughput (7.62\,Mbps).
PCF-MORL is the only method that simultaneously achieves low VR \emph{and} competitive throughput (9.20\,Mbps).

\input{tabs/tab4_pareto}

Table~\ref{tab:pareto} shows aggregate Pareto front quality.
PCF-MORL dominates all metrics: HV$=$324{,}849 (35\% higher than the best baseline), EU$=$7.99 (the only positive EU, indicating net positive scalarized return), and $|$CCS$|{=}$15 distinct QoS operating points.
Threshold baselines each contribute a single fixed operating point; Scalarized DQN provides 3 (one per trained weight).
...
Fig.~\ref{fig:pareto} visualizes the Pareto front projection.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figs/fig2_pareto_front.pdf}
\caption{Pareto front projection ($r_1$ throughput vs.\ $r_2$ delay). PCF-MORL (circles) spans a dense Pareto front across multiple QoS trade-offs; baselines produce isolated clusters.}
\label{fig:pareto}
\end{figure}
...
Results show ZSR$=$0.99, meaning instant adaptation achieves 99\% of dedicated-training quality across all test preferences.
TTM for fine-tuning: $\sim$500 episodes ($\sim$4~hours on ns-3).
TTM for retraining from scratch: $\sim$2{,}000 episodes ($\sim$17~hours).

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figs/fig3_adaptation_curves.pdf}
\caption{Adaptation curves after QoS preference switch for three test weights. GPI (blue) achieves near-oracle performance instantly; Fine-tune and Retrain require hundreds of episodes to converge.}
\label{fig:adaptation}
\end{figure}

Fig.~\ref{fig:adaptation} shows learning curves after a preference switch.
GPI adaptation (blue) achieves 99\% of oracle quality (brown) from episode~0---a dot-product computation taking ${<}5$\,ms.
Fine-tuning (orange) reaches comparable quality after $\sim$500~episodes, while retraining from scratch (green) requires $\sim$2{,}000~episodes.
...
Fig.~\ref{fig:speedup} shows the scaling results.
At $P{=}8$, we achieve $4.1\times$ speedup ($\eta{=}0.51$); at $P{=}16$, $5.4\times$ ($\eta{=}0.34$).
Fitting Amdahl's Law $S(P) = 1/(f + (1{-}f)/P)$ yields serial fraction $f{=}0.85$, consistent with the 7\% synchronization overhead and per-worker initialization costs.

\begin{figure}[t]
\centering
\includegraphics[width=0.85\columnwidth]{figs/fig4_speedup.pdf}
\caption{Parallel scaling: measured speedup vs.\ ideal and Amdahl's law ($f{=}0.85$). Error bars show 95\% CI across 5 seeds.}
\label{fig:speedup}
\end{figure}

\textbf{SF sharing ablation.}
At $P{=}8$, we compare four SF sharing strategies: no sharing (independent workers), CCS union with $K{=}200$ (default), frequent sharing ($K{=}50$), and rare sharing ($K{=}1{,}000$).
CCS union with $K{=}200$ achieves the best HV, confirming the MORL-specific diversity bonus from Section~\ref{sec:diversity}.
No sharing achieves only 75\% of the shared HV despite identical compute budget.
Frequent sharing ($K{=}50$) incurs 12\% overhead with marginal quality gain; rare sharing ($K{=}1{,}000$) loses 12\% HV due to stale SFs.

\textbf{Sync interval trade-off.}
Fix $P{=}8$, vary $K \in \{50, 100, 200, 500, 1000\}$.
$K{=}200$ is the sweet spot: lowest wall-clock time with highest HV.
...
Using Successor Features with GPI, a single trained model adapts instantly to changing operator QoS priorities---a ${<}5$\,ms dot-product computation achieving ${\geq}$99\% of dedicated-training quality without retraining.
...
Experiments on a 14-UE smart factory scenario demonstrate 51\% VR reduction and 31\% TTR improvement over the best industrial-grade threshold baseline (A3 Hysteresis), with 15 QoS operating points versus 1--3 for baseline methods.
